...
//     const updateUnreadMessages = async (userId) => {
//       try {
//         const unreadCount = await Message.countDocuments({ receiver: userId, isRead: false });
//         io.to(userId).emit('unreadMessagesCount', unreadCount);
//       } catch (error) {
//         console.error('Error fetching unread messages:', error);
//       }
//     };

//     // User joins their room based on userId after authentication
...
//           const undeliveredMessages = await Message.find({ receiver: userId, isDelivered: false });
//           undeliveredMessages.forEach((message) => {
//             socket.emit('receiveMessage', message);
//             message.isDelivered = true;
//             message.save(); // Update the delivery status
//           });
...
//     // Fetch message history and mark messages as read
//     socket.on('getMessageHistory', async ({ userId, otherUserId }, callback) => {
//       try {
//         const userObjectId = new mongoose.Types.ObjectId(userId);
//         const otherUserObjectId = new mongoose.Types.ObjectId(otherUserId);
...
//                console.log(messages)
//         callback({ status: 'success', messages });
//         updateUnreadMessages(userId); // Update unread count for the current user
//       } catch (error) {
//         console.error('Error fetching message history:', error);
//         callback({ status: 'failed', message: 'Error fetching message history' });
//       }
